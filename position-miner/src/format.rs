...
//! 116 bytes you can rebuild the board, name the two humans who played it, say
...
//!
//! record (116 bytes)
//!   0    id            u64      FNV-1a of the packed position (see `Position::id`)
//!   8    occupied      u64      bitboard; bit i set = square i is occupied (a1=0 … h8=63)
//!   16   pieces        [u8; 4]  8 nibbles, one per set bit of `occupied`, ascending
//!   20   stm           u8       0 = white to move, 1 = black
//!   21   castling      u8       bitmask: 1 K, 2 Q, 4 k, 8 q
//!   22   ep_square     u8       en-passant target square, 255 = none
//!   23   halfmove      u8       halfmove clock, saturating at 255
//!   24   fullmove      u16      fullmove number
//!   26   ply           u16      0-based ply index of this position within the game
//!   28   game_plies    u16      total plies the game lasted
//!   30   eval_cp       i16      lichess [%eval] in centipawns, white POV
//!   32   verified_cp   i16      stage-2 engine eval, white POV; EVAL_UNSET if not verified
//!   34   wdl_win       u16      stage-2 win  per-mille, side-to-move POV; 0 if unverified
//!   36   wdl_draw      u16      stage-2 draw per-mille
//!   38   wdl_loss      u16      stage-2 loss per-mille
//!   40   white_elo     u16
//!   42   black_elo     u16
//!   44   tc_initial    u16      time control base, seconds
//!   46   tc_increment  u16      time control increment, seconds
//!   48   utc_time      i64      game start, unix seconds
//!   56   game_id       [u8; 8]  lichess game id (base62), NUL padded
//!   64   winner        u8       0 = white, 1 = black (draws are never stored)
//!   65   termination   u8       see `Termination`
//!   66   flags         u8       bit 0: mate score in `eval_cp`
//!   67   _reserved     u8
//!   68   white_name    [u8; 20] UTF-8 username, NUL padded (lichess caps at 20)
//!   88   black_name    [u8; 20]
//!   108  second_cp     i16      stage-2 second-best root move, white POV
//!   110  drop_cp       i16      stage-2 best minus second-best, side-to-move POV
//!   112  legal_moves   u8       root moves the engine reported (capped at MultiPV)
//!   113  losing_moves  u8       of those, how many lose by >= 300cp
//!   114  holding_moves u8       of those, how many stay within 50cp of the best
//!   115  _reserved2    u8
//! ```
//!
...
impl Record {
    pub fn encode(&self) -> [u8; RECORD_SIZE] {
        ...
        b[16..20].copy_from_slice(&self.pieces);
        b[20] = self.stm;
        b[21] = self.castling;
        b[22] = self.ep_square;
        b[23] = self.halfmove;
        put!(b, 24, self.fullmove);
        put!(b, 26, self.ply);
        put!(b, 28, self.game_plies);
        put!(b, 30, self.eval_cp);
        put!(b, 32, self.verified_cp);
        put!(b, 34, self.wdl_win);
        put!(b, 36, self.wdl_draw);
        put!(b, 38, self.wdl_loss);
        put!(b, 40, self.white_elo);
        put!(b, 42, self.black_elo);
        put!(b, 44, self.tc_initial);
        put!(b, 46, self.tc_increment);
        put!(b, 48, self.utc_time);
        b[56..64].copy_from_slice(&self.game_id);
        b[64] = self.winner;
        b[65] = self.termination;
        b[66] = self.flags;
        b[68..88].copy_from_slice(&self.white_name);
        b[88..108].copy_from_slice(&self.black_name);
        put!(b, 108, self.second_cp);
        put!(b, 110, self.drop_cp);
        b[112] = self.legal_moves;
        b[113] = self.losing_moves;
        b[114] = self.holding_moves;
        b
    }

    pub fn decode(b: &[u8]) -> Record {
        let mut pieces = [0u8; 4];
        pieces.copy_from_slice(&b[16..20]);
        let mut game_id = [0u8; 8];
        game_id.copy_from_slice(&b[56..64]);
        let mut white_name = [0u8; NAME_LEN];
        white_name.copy_from_slice(&b[68..88]);
        let mut black_name = [0u8; NAME_LEN];
        black_name.copy_from_slice(&b[88..108]);
        Record {
            id: get!(b, 0, u64),
            ...
            ep_square: b[22],
            halfmove: b[23],
            fullmove: get!(b, 24, u16),
            ply: get!(b, 26, u16),
            game_plies: get!(b, 28, u16),
            eval_cp: get!(b, 30, i16),
            verified_cp: get!(b, 32, i16),
            wdl_win: get!(b, 34, u16),
            wdl_draw: get!(b, 36, u16),
            wdl_loss: get!(b, 38, u16),
            white_elo: get!(b, 40, u16),
            black_elo: get!(b, 42, u16),
            tc_initial: get!(b, 44, u16),
            tc_increment: get!(b, 46, u16),
            utc_time: get!(b, 48, i64),
            game_id,
            winner: b[64],
            termination: b[65],
            flags: b[66],
            white_name,
            black_name,
            second_cp: get!(b, 108, i16),
            drop_cp: get!(b, 110, i16),
            legal_moves: b[112],
            losing_moves: b[113],
            holding_moves: b[114],
        }
...
mod tests {
    ...
    fn record_round_trips() {
        ...
        rec.id = rec.id;
        let back = Record::decode(&rec.encode());
        assert_eq!(back.second_cp, -340);
        assert_eq!(back.drop_cp, 348);
        assert_eq!(back.legal_moves, 22);
        assert_eq!(back.losing_moves, 19);
        assert_eq!(back.holding_moves, 2);
        assert_eq!(back.id, rec.id);
        assert_eq!(back.occupied, rec.occupied);
        assert_eq!(back.pieces, rec.pieces);
        assert_eq!(back.utc_time, rec.utc_time);
